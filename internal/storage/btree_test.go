...
func TestAscendingInsertAndGetInternal(t *testing.T) {
	db, err := engine.Open("test_insert_internal.db")
	if err != nil {
		t.Fatal(err)
	}
...
func TestAscendingInsertDescendingDeleteInternal(t *testing.T) {
	db, err := engine.Open("test_ordered_internal.db")
	if err != nil {
		t.Fatal(err)
	}
